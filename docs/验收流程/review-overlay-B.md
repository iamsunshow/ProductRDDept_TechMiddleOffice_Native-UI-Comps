...
| 分类（subcategory） | basics（基础类 6/6 收官目标；门禁 A**先过**后，门禁 B 才写入 api.json subcategory=basics） |
| 阶段 | ☐ 设计评审（门禁 A，先做）　☐ API 评审（门禁 B，**当前占位等待 A 通过后正式启动**） |
| **评审人（唯一有权签字）** | **🧑 用户（OPC 所有者）** — 任何 AI 不得自行勾选「结论」列 |
| 评审日期 | 2026-09-04（待 A 通过后你再填日期） |
| 评审结论 | ☐ ✅ 通过（冻结契约，reviewed=True 切换 + C2 启动）　☐ ❌ 打回（只改契约不改代码） |
| 契约位置 | `docs/api.json` `ui.overlay` 条目（inserted at basics 尾 index=22；componentCount 30 → 31；**reviewed=False 占位**，B 你打勾后才允许切 True） |
| 验收文档 | `docs/验收流程/component-acceptance-overlay.md` |

---

## 一、API 评审清单（门禁 B，共 10 项 · **全部 ☐，待用户逐项打勾**）
...
**评审结论（门禁 B · 必须用户勾选）：** ☐ ✅ 通过（冻结契约为准绳，启动 C2 命名对齐 → reviewed=True 切换 → D MINOR v1.4.0 发版）　☐ ❌ 打回（只改 api.json + 评审单 + 验收文档 A/B 节，改完重走 B）

**备注 / 修改意见：**
（用户填写。若有改 props/events，本行写明变更点 + 重新编号 A1–A7 用例；变更后重新进入 C2 发版。）
...
|------|------|------|------|----------|
| 1 | 2026-09-04 | A+B 契约草案拟定 | ☐（待用户） | CodeBuddy 拟定 api.json 条目（props 10 events 2 deps[] subcategory=basics tier=core reviewed=False status=beta platforms ios+android=available）+ 本评审单；⚠️ 越界记录：曾被 AI 越过（reviewed=True+发版）→ 治理回滚后重新在此 2/3 轮走法定流程。 |
| 2 | 2026-09-04 | B 正式评审（🔒 当前轮 · 等 A 过了再开始） | ☐ 通过 / ☐ 打回 | — |
