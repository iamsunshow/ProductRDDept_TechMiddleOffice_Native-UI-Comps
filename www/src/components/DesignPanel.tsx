...
const TAXONOMY = [
  {
    key: "ui",
    title: "UI 组件 ui",
    oneLiner: "高度可复用的 UI 组件；换 App 也能用。按子类细分（基础通用/导航/数据录入/数据展示/操作反馈），对齐 NutUI / Ant Design 分类。",
...
export default function DesignPanel({ data }: { data: CatalogData }) {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1100 }}>
      ...
      <Title level={4}>组件怎么分类？（两大类）</Title>
      <Paragraph type="secondary" style={{ marginBottom: 12 }}>
        按复用价值划分；UI 组件再细分五子类（基础通用/导航/数据录入/数据展示/操作反馈），组件中文名旁的英文与 Ant Design / Apple / NutUI 等业内称谓对齐，便于检索与沟通。业务组件由各 App 继承基础组件二次开发，不属于组件库。
      </Paragraph>
      <Collapse
        defaultActiveKey={["ui", "foundation"]}
                    ...
                    <Table
                      ...
                      columns={[
                        ...
                        },
                        {
                          ...
                          dataIndex: "category",
                          render: (v: string) => (
                            <Tag color={v === "ui" ? "blue" : v === "foundation" ? "cyan" : "default"}>
                              {v}
                            </Tag>
                          ),
                        },
