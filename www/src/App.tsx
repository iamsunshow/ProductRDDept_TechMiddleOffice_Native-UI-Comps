  ...
  const menuItems: MenuProps["items"] = useMemo(() => {
    ...
    return data.categoryOrder
      .map((cat) => {
        ...
        const subKeys = Object.keys(subCats).filter((sk) =>
          children.some((c) => c.subcategory === sk),
        );
        // ui 大类：按五子类分组（三级菜单）
        if (subKeys.length > 0) {
          return {
            ...
            children: subKeys.map((sk) => ({
              key: `${cat}.${sk}`,
              title: subCats[sk]?.title ?? sk,
              label: `${subCats[sk]?.title ?? sk}（${children.filter((c) => c.subcategory === sk).length}）`,
              children: children.filter((c) => c.subcategory === sk).map(compItem),
            })),
          };
        }
        // 其他大类：平铺
        return {
          key: cat,
          label: `${meta?.title ?? cat}（${children.length}）`,
          children: children.map(compItem),
        };
      })
          ...
          {tab === "components" ? (
            ...
            >
              <Menu
                ...
                items={menuItems}
                onClick={({ key }) => {
                  if (data.components.some((c) => c.id === key)) setSelectedId(key);
                }}
              />
            </Sider>
          ) : null}
