...
export default function ContractMatrix({
  ...
  data: {
    ...
    subcategories: Record<string, Record<string, { title: string }>>;
    ...
    return data.components
      ...
      .map((c) => ({
        ...
        category: c.subcategory
          ? `${data.categories[c.category]?.title ?? c.category} · ${data.subcategories[c.category]?.[c.subcategory]?.title ?? c.subcategory}`
          : (data.categories[c.category]?.title ?? c.category),
        status: c.status,
