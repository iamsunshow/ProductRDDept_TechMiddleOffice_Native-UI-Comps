...
| 样式 Token 铁律 | `docs/README.md` + `docs/design-token.json`（真实数据） |
| 版本号 | `docs/ui-version.json` |
...
- **汇报与验证组件都必须带版本号（强制，2026-09-01 新增）**：任何讨论/汇报/改动/验证某个组件（如 Cell）时，必须先说明该组件当前所基于的组件库版本号（`docs/ui-version.json` 的 `version`，双端同版），**验证时明确"验证的是组件库哪个版本"**。若本会话中改动过该组件，汇报/验证必须标注"已提升/基于的版本号 + 改动内容"，避免用户无法区分当前组件是哪个版本。版本号有变则同步更新 CHANGELOG 与 ui-version.json，双端一致后再验证。未标注版本号即视为汇报/验证不完整。
