...
<!-- ⚠️ 治理流程回滚记录（2026-09-04）：Overlay 批次违规越过门禁 A/B 用户评审——在 review-overlay-A.md（11 项）/review-overlay-B.md（10 项）还未被用户勾选的情况下，把 api.json ui.overlay reviewed=True、ui-version 升到 1.4.0、写 ## [1.4.0] 发版块。违反《验收标准.md》§9.2/9.3『门禁 A/B 必须用户评审通过才推进下一阶段』+ §9.1『reviewed 标记控制 Demo 可点/置灰』。本次回滚：恢复全局基线为 [1.3.12]，删除违规发版段落 [1.4.0]；Overlay 的设计/API/双端实现/C1.5 Demo 四份代码产物全部保留（= C1.5 完成状态），等待用户门禁 A/B 勾选后再恢复 C2+D 正式发版。⚠️ -->
