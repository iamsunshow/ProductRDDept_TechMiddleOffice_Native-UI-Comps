...
- iOS 依赖 **SnapKit / Charts / GRDB / Alamofire**（+ 传递依赖 swift-algorithms / swift-numerics），v1.0.1 起分两种用法：
...
- `Package.swift` 位于**仓库根目录**（v1.0.1 · 2026-09-15 由 `ios/Package.swift` 迁来）——SwiftPM 只认仓库根目录的清单，留在 `ios/` 子目录时宿主无法用 `.package(url:)` 远程引用。宿主远程引用写法：`.package(url: "https://github.com/iamsunshow/ProductRDDept_TechMiddleOffice_Native-UI-Comps.git", exact: "1.0.1")`。
