...
export default function HomePage({ data }: { data: CatalogData }) {
  ...
  return (
    <div style={{ padding: "24px 32px 48px", maxWidth: 1080, margin: "0 auto" }}>
      ...
      {/* 分类概览 */}
      <SectionTitle>组件分类</SectionTitle>
      <Row gutter={[16, 16]}>
        {data.categoryOrder.map((cat) => {
          ...
          return (
            <Col xs={24} md={cat === "ui" ? 24 : 8} key={cat}>
              <Card
                title={
                  <span>
                    {metaCat?.title ?? cat}
              ...
              >
                ...
                {subKeys.length > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 12 }}>
                    {subKeys.map((sk) => {
                      const subMeta = subCats[sk];
                      const subComps = comps.filter((c) => c.subcategory === sk);
                      return (
                        <div
                          key={sk}
                          style={{
                        ...
                        >
                          <div style={{ fontWeight: 600, fontSize: 13 }}>
                            {subMeta?.title ?? sk}
                            <Tag style={{ marginLeft: 6 }}>{subComps.length}</Tag>
                          </div>
                          <div style={{ fontSize: 12, color: "rgba(0,0,0,0.45)", marginTop: 4 }}>
                            {subComps.map((c) => c.name).join(" · ")}
                          </div>
                        </div>
                      );
                    })}
