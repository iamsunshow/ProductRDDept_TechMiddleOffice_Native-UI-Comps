# TMO 组件库 · 设计规范（Design + Token）

> 双端视觉统一铁律。**唯一数据源 = `docs/design-token.json`**（颜色 / 字号 / 圆角 / 间距 / 行距）。
> 所有颜色/间距/圆角/字号/阴影，双端一律读取 Token，禁止硬编码。
> 本文件说明 Token 结构、双端自动生成、平台差异白名单。
> 2026-08-30 由原 `design-spec.md` + `design-token-README.md` 二合一。

---

## 一、唯一数据源原则

- 唯一 Token 源文件：`docs/design-token.json`（版本库内权威 JSON），由 `catalog/` 收敛前的 `design-token/tokens.json` 迁移而来（方案 B）。
- 文档站（`www/scripts/generate_data.py`）构建时读取此文件，作为「设计面板」的 token 数据源。
- 组件 metadata（`docs/api.json`）中每个组件的 `visual_tokens` 引用此处的 token 名。
- 脚本自动生成：
  - iOS：Swift 样式常量（`gen_token_ios`）
  - Android：xml/kt 样式资源（`gen_token_android`）
- **改一处，双端同步生效**；两端代码永不手写样式值。
- 新样式必须先加 Token，再在双端使用；禁止直接写死数值。
- 双端代码从该文件生成样式常量；映射位置见两端源码 `Foundation/Design`。
...
> 各组件状态表现详见 `docs/开发规则.md` 第八节「双端 API 契约」对应章节。
