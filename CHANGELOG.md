...
- **第二轮编译报错：`Cannot find 'BarChartView' / 'ProgressCircleView' / 'ShortcutBarView' in scope`**（上轮只补了 DemoApp 3 个 Showcase 注册，漏了 SharedUI/Components 库内 3 个 View 注册——demo 工程走 PBXBuildFile 直接 link 模式，不走 SPM，库内每个 .swift 都得手工登记）。
  ...
  - **处置**：给 3 个 View 各加 4 处 `.pbxproj` 注册（PBXBuildFile / PBXFileReference / PBXGroup Components 子 group 字母序 / PBXSourcesBuildPhase），共 12 处 UUID全新生成：
    ...
    - `ShortcutBarView`: `BA00A5C7D8E9F012345678B5` (FileRef) + `BA00A6C8D9EAF123456789B6` (BuildFile)
    - Components 子 group 字母序位置：BarChartView 在 BackTopButton 后；ProgressCircleView 在 PriceView 后；ShortcutBarView 在 SearchBarView 后。
...
- **第三轮编译报错：3 个 Showcase 缺 `import UIKit / import SnapKit` + BarChartView 闭包漏 `self.`**（第二轮只补了 .pbxproj 文件登记，没修源码本身的漏报）。
  ...
  - **处置**（4 处 = 3 Showcase 加 import + 1 View 加 self）：
    ...
    - `demo/ios/DemoApp/ProgressCircleShowcase.swift`：文件首插 `import UIKit / import SnapKit`
    - `demo/ios/DemoApp/ShortcutBarShowcase.swift`：文件首插 `import UIKit / import SnapKit`
...
- **v1.0.2 public 化同源遗漏：iOS `CategoryPickerView` 行 10/48 + `PeriodTabsView` 行 8 编译 `public (var|func) ... uses an internal type`**。
  ...
  - **处置**（保持 v1.0.2 public 化设计意图原样不动）：
    ...
    - 把 `BookkeepingCategory.swift` + `ChartsPeriod.swift` 各注册到 demo `.pbxproj` 4 处（共 8 处，UUID 全新生成 `B1C2D3E4F5A6B7C8D9E0F1A2B` + `A1B2C3D4E5F678A0B1C2D3E4` + `D1E2F3A4B5C6D7E8F9A0B1C2D` + `C1D2E3F4A5B6C7D8E9F0A1B2C`，`plutil -lint` 校验 OK）。
  - **包边界不动**：`Package.swift` exclude 仍保留 `CategoryPickerView.swift` + `PeriodTabsView.swift`，但 `BookkeepingCategory.swift` + `ChartsPeriod.swift` 作为数据契约类型保留在 SP target——宿主 `import TMONativeUIComps` 即可获得。
  - **iOS Components 总数** 85 → **87**（新增 `BookkeepingCategory.swift` + `ChartsPeriod.swift`）。
...
- iOS：`xcodebuild -scheme tmo-native-ui-comps -destination 'generic/platform=iOS Simulator' -derivedDataPath /tmp/lib-dd build` BUILD SUCCEEDED（待 ① 编译验证命令实际执行后回执）。
- Android：`./gradlew :components:compileDebugKotlin` BUILD SUCCESSFUL（待 ② 编译验证命令实际执行后回执）。
- api.json 4 件契约无变化（components 总数仍 92）。
...
- **iOS `PopupView.swift:237` 编译报错 `No exact matches in call to instance method 'append'`**（用户 2026-09-13 Xcode 反馈）。
  - 根因：v1.9.24 新增的容器高度等式是 `NSLayoutConstraint`，而 `contentConstraints` 数组元素类型是 SnapKit 的 `Constraint`（同函数内 `prepareConstraints` 产出），两者不能混装进同一数组。
  - 修复：新增独立属性 `containerHeightConstraint: NSLayoutConstraint?` 单独持有，`replaceContent()` 开头先 `isActive = false` 再置 nil（与 `contentConstraints` 一同释放），不再 append 进数组。
  - 验证：`xcodebuild -scheme ZhiqihuayunDemo -sdk iphonesimulator -configuration Debug` → `BUILD SUCCEEDED`（PopupView.swift 已重新编译）。
...
- Popup 居中弹层宽度算错导致文案被截断（Android 显示「大圆」，iOS 完整）。
  - 根因：`maxCenterWidth` 用 `LocalDensity.toDp()` 把 dp 当 px 又除一次密度，max 反压 min=240 → 弹层被压窄。
  - 同步修正：Demo1/6/7/8 内容最小宽度 240/280dp+16dp 内边距 → 208dp（弹层 240dp 与 iOS 一致）；容器内容改 TopCenter 居中；Demo6/8 说明文案 dp→pt。
  - 实机验证：Pixel 模拟器 Demo1「常规弹出层内容」、Demo8「大圆角弹出层（24pt）」均完整显示。

## \[1.9.21] - 2026-09-13（Overlay 用户验收通过状态同步）

### Fixed

- **用户验收通过状态同步**：用户原话「Overlay：已通过」，双端 Demo `passed=true` 已同步：
  - iOS `DemoShowcases.swift` 第 39 行 `pending=true` → `passed=true`。
  - Android `MainActivity.kt` 第 290 行 `pending=true` → `passed=true`。
  - `docs/组件进度.md` 第 6 行状态从「📋⚠️重点修复」改为「✅」，备注更新为 v1.9.16~v1.9.20 共 5 版本修复 8 项双端一致+回归测试 4 例（OverlayTest 14/14 全绿，台账 #69-#72）。
...
- **iOS Demo4 手柄通栏**：handle 直接作为 stack arrangedSubview，`stack.alignment = .fill` 覆盖 handle `width=40` 约束导致通栏（Android `Spacer(40.dp).align(CenterHorizontally)` 正常）。修复=用 `handleWrap`(UIView) 包裹 handle，`handleWrap` fill 宽度、handle 在 `handleWrap` 内 `centerX` 居中保持 40×4，双端手柄均为 40dp 居中。
- 验证：iOS `xcodebuild` BUILD SUCCEEDED。

## \[1.9.18] - 2026-09-13（Overlay Demo5 描述文案居左+行高标准化）

### Fixed

- **iOS Demo5 描述居左不生效**：`textAlignment = .left` 在 Overlay 布局中未生效（用户反馈仍居中）。修复=改用 `NSAttributedString` + `NSMutableParagraphStyle(alignment: .left, lineSpacing: 4)` 强制段落样式，同时统一行高。
- **双端行高不一致**：iOS `lineSpacing = 4`（font.lineHeight≈16 + 4 = 20pt）/ Android `lineHeight = 20.sp`，双端行高均≈20pt，视觉一致。
- 验证：Android `./gradlew :app:assembleDebug` BUILD SUCCESSFUL；iOS `xcodebuild` BUILD SUCCEEDED。
