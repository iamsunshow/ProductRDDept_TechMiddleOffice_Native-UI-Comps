...
│   │   ├── flow.html         #   验收流程图（H5，浏览器打开）
│   │   ├── 验收模板.md        #   验收文档模板（设计细节/设计测试/API细节/API测试 四要素）
│   │   └── 评审清单.md        #   评审意见单（设计/API 验收打勾模板）
│   ├── design-spec/          #   设计规格 H5 页（如 cell-design-spec.html，门禁 A 输入资产）
│   └── CHANGELOG.md          #   版本日志
├── ios/                  # iOS 组件库源码（SPM）
│   ├── Package.swift
...
├── www/                  # 组件文档站（React+Vite+AntD，构建时读 docs/api.json 生成）
│   └── scripts/generate_data.py
├── scripts/              # 工具脚本
│   ├── embed.py          #   生成检索向量
│   ├── search.py         #   自然语言搜组件
│   ├── merge_contracts_into_catalog.py  # API契约并入 docs/api.json
...
- 阶段二（API 契约收口 + 目录收敛）：✅ 组件 metadata 已收口为 `docs/api.json`（含 API 契约 props/events/demos/note），样式数据收口为 `docs/design-token.json`，版本号收口为 `docs/ui-version.json`；`catalog/`、`design-token/` 目录已并入 `docs/`，`docs-site/` 已更名 `www/`，文档站构建时读 `docs/api.json` 生成，不再维护多份数据源。
